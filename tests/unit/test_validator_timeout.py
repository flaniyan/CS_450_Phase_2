import json, subprocess, tempfile, os, textwrap, pathlib, sys
...
def run(code, payload, env_overrides=None, timeout=8):
    ...
    try:
        env=os.environ.copy(); env.setdefault("VALIDATOR_TIMEOUT_SEC","2"); env.setdefault("VALIDATOR_MEMORY_MB","64")
        if env_overrides: env.update(env_overrides)
        p = subprocess.run(["python3","-I","-S",DRIVER,path,json.dumps(payload)],
