...
class AWSTestSuite:
    ...
    def test_s3_bucket(self):
        ...
        try:
            ...
            self.log_test(
                "S3 Bucket Access",
                True,
                f"Bucket {self.artifacts_bucket} is accessible",
            )
            ...
            try:
                ...
                self.log_test(
                    "S3 Bucket Encryption", True, "Bucket has encryption enabled"
                )
            except:
                self.log_test(
                    "S3 Bucket Encryption", False, "Bucket encryption not configured"
                )
        ...
        try:
            ...
            for table_name in expected_tables:
                if table_name in existing_tables:
                    ...
                    self.log_test(
                        f"DynamoDB Table {table_name}", False, "Table does not exist"
                    )
                ...
                if services["serviceArns"]:
                    ...
                    if running_count == desired_count:
                        ...
                        self.log_test(
                            "ECS Service",
                            False,
                            f"Service not fully running ({running_count}/{desired_count})",
                        )
                else:
    ...
    def test_secrets_manager(self):
        ...
        try:
            ...
            if "VersionIdsToStages" in response and "AWSCURRENT" in str(
                response["VersionIdsToStages"]
            ):
                self.log_test(
                    "Secrets Manager", True, "JWT secret exists with current version"
                )
            else:
                self.log_test(
                    "Secrets Manager", False, "JWT secret exists but no current version"
                )
    ...
    def test_end_to_end_workflow(self):
        ...
        try:
            ...
            self.log_test(
                "End-to-End Workflow",
                True,
                "Workflow test placeholder - implement with actual URLs",
            )
