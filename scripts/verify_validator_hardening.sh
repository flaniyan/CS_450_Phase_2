...
time curl -s -X POST "$base/validate" -H 'Content-Type: application/json' \
  -d "{\"script\": \"$(python - <<'P';import json;print(json.dumps(open('/tmp/loop.py').read()));P)\"}" | jq .
...
curl -s -X POST "$base/validate" -H 'Content-Type: application/json' \
  -d "{\"script\": \"$(python - <<'P';import json;print(json.dumps(open('/tmp/mem_bomb.py').read()));P)\"}" | jq .
...
P
curl -s -X POST "$base/validate" -H 'Content-Type: application/json' \
  -d "{\"script\": \"$(python - <<'P';import json;print(json.dumps(open('/tmp/big.txt').read()));P)\"}" | jq .
