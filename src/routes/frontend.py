...
def register_routes(app):
    ...
    @app.get("/directory")
    def directory(request: Request, q: str | None = None, name_regex: str | None = None, model_regex: str | None = None, version_range: str | None = None, version: str | None = None):
        if not templates:
            return {"message": "Frontend not found. Ensure frontend/templates exists."}
        packages = []
        try:
    ...
    @app.post("/upload")
    def upload_post(request: Request, file: UploadFile = File(...), model_id: str = None, version: str = None):
    ...
    @app.get("/size-cost")
    def size_cost(request: Request, name: str | None = None, version: str | None = None):
        if not templates:
            return {"message": "Frontend not found. Ensure frontend/templates exists."}
        size_data = None
        if name:
