...
class DatasetAndCodeMetric:
    ...
    def score(self, meta: dict) -> MetricValue:
        ...
        if readme_text:
            ...
            dataset_keywords = ['dataset', 'data', 'training data', 'corpus', 'benchmark']
            if any(keyword in readme_text for keyword in dataset_keywords):
                score += 0.3
            
            # Look for specific well-known datasets
            known_datasets = ['imagenet', 'coco', 'openimages', 'wikipedia', 'common crawl', 
            ...
            code_keywords = ['code', 'implementation', 'source', 'repository', 'github', 'script']
            if any(keyword in readme_text for keyword in code_keywords):
                score += 0.2
            
            # Look for example usage or demo code
            example_keywords = ['example', 'demo', 'tutorial', 'usage', 'quickstart', 'getting started']
