...
class RampUpMetric:
    ...
    def score(self, meta: dict) -> MetricValue:
        ...
        if readme_text:
            ...
            if any(keyword in readme_text for keyword in ['install', 'usage', 'example', 'quickstart', 'getting started']):
                score += 0.2
            if any(keyword in readme_text for keyword in ['api', 'documentation', 'docs']):
                score += 0.1
