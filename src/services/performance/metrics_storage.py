        ...
        with table.batch_writer() as batch:
            for idx, metric in enumerate(metrics):
                try:
                    ...
                    metric_id = f"{metric.get('client_id', 0)}_{idx}_{uuid.uuid4().hex[:8]}"
                    ...
                    item = {
                        ...
                        "request_latency_ms": float(metric.get("request_latency_ms", 0)),
...
def publish_metrics_to_cloudwatch(
    ...
    try:
        ...
        latencies = [m.get("request_latency_ms", 0) for m in metrics if m.get("status_code") == 200]
        ...
        throughput_bps = total_bytes / total_duration_seconds if total_duration_seconds > 0 else 0
        ...
        logger.error(f"Unexpected error publishing metrics: {type(e).__name__}: {str(e)}")
        return False
...
def store_and_publish_metrics(
    ...
    cloudwatch_success = publish_metrics_to_cloudwatch(run_id, metrics, total_duration_seconds)
