...
def _run_load_generator_async(run_id: str, base_url: str, num_clients: int, model_id: str, version: str, duration_seconds: Optional[int]):
    """
    ...
    try:
        ...
        if run_id in _workload_runs:
            ...
            _workload_runs[run_id]["completed_at"] = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
        ...
        logger.error(f"Error in load generator for run_id={run_id}: {str(e)}", exc_info=True)
        if run_id in _workload_runs:
